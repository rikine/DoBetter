...
/*
You can conform this protocol along with CodingKey to make
JSONMDecoder inject parent json object accessible by `selfKeys`.
See InstrumentIdentifier.swift
*/

protocol AllKeysProvider {
    static var allKeys: [String] { get }
}

protocol InjectSelfKeys: CaseIterable, AllKeysProvider {}

extension InjectSelfKeys where AllCases.Element: RawRepresentable, AllCases.Element.RawValue == String {
    static var allKeys: [String] { Self.allCases.map(\.rawValue) }
}

extension DecodingError {
    /*
    Throwing this error means some of your guts missing values,
    so you are not able to construct valid instance of your decodable
    but if it an optional property (i.e. decoded by decodeIfPresent)
    rest of the parent decodable can keep going
    */
    private static let keyNotFoundMayBeIgnoredCodingKey = "keyNotFoundMayBeIgnored"
    static let keyNotFoundMayBeIgnored =
        keyNotFound(_JSONKey(stringValue: keyNotFoundMayBeIgnoredCodingKey)!, Context(codingPath: [], debugDescription: ""))

    /*
    Use this function to compare any Error with keyNotFoundMayBeIgnored, as Error is not equatable in general.
    */
    fileprivate static func canIgnore(error err: Error) -> Bool {
        guard let error = err as? DecodingError else { return false }
        switch error {
        case .keyNotFound(_, _), .valueNotFound: return true
...
open class JSONMDecoder {
    ...
    open func decode<T: Decodable>(_ type: T.Type, from data: Data) throws -> T {
        ...
        case let .right(topLevel):
            let decoder = _JSONMDecoder(referencing: topLevel, options: self.options)
            guard let value = try decoder.unbox(topLevel, as: type) else {
                throw DecodingError.valueNotFound(type, DecodingError.Context(codingPath: decoder.codingPath, debugDescription: "The given data did not contain a top-level value."))
            }
...
private class _JSONMDecoder: Decoder {
    // MARK: Properties
    /// The decoder's storage.
    fileprivate var storage: _JSONDecodingStorage

    /// Used to decode Int-s
    private let numberFormatter: NumberFormatter = {
        let formatter = NumberFormatter()
        // To decode Int from Decimal, 'cause Decimals from APL are always separated by '.'
        formatter.decimalSeparator = "."
        return formatter
    }()

    /// Options set on the top-level decoder.
    fileprivate let options: JSONMDecoder._Options
    ...
    fileprivate init(referencing container: JSON, at codingPath: [CodingKey] = [], options: JSONMDecoder._Options) {
    ...
    public func container<Key>(keyedBy type: Key.Type) throws -> KeyedDecodingContainer<Key> {
        ...
        var injectRoot = dictionary

        if let _type = type as? AllKeysProvider.Type {
            _type.allKeys.forEach {
                if $0.starts(with: "$") {
                    injectRoot[$0] = storage.topContainer
                }
            }
        }


        let container = _JSONKeyedDecodingContainer<Key>(referencing: self, wrapping: injectRoot)
...
private struct _JSONKeyedDecodingContainer<K: CodingKey>: KeyedDecodingContainerProtocol {
    ...
    private let decoder: _JSONMDecoder
    ...
    fileprivate init(referencing decoder: _JSONMDecoder, wrapping container: [String: JSON]) {
        ...
        case .convertFromSnakeCase:
            ...
            self.container = Dictionary(container.map {
                key, value in
                (JSONMDecoder.KeyDecodingStrategy._convertFromSnakeCase(key), value)
            }, uniquingKeysWith: { (first, _) in first })
        case .custom(let converter):
            self.container = Dictionary(container.map {
                key, value in
                (converter(decoder.codingPath + [_JSONKey(stringValue: key, intValue: nil)]).stringValue, value)
            }, uniquingKeysWith: { (first, _) in first })
        }
    ...
    public func decode<T: Decodable>(_ type: T.Type, forKey key: Key) throws -> T {
        guard let entry = self.container[key.stringValue] else {
            if let type = type as? Synthesizable.Type { return type.synthesize() as! T }
            throw DecodingError.keyNotFound(key, DecodingError.Context(codingPath: self.decoder.codingPath, debugDescription: "No value associated with key \(_errorDescription(of: key))."))
        }
        ...
        do {
            ...
            if let type = type as? Synthesizable.Type {
                return type.synthesize() as! T
            } else {
                throw error
            }
        }
    ...
    func decodeIfPresent<T>(_ type: T.Type, forKey key: K) throws -> T? where T: Decodable {
        ...
        do {
            ...
            guard type is RecoverFromUnknownValueAsNil.Type ||
                      DecodingError.canIgnore(error: error)
            else { throw error }
            return nil
        }
    }
    ...
    private func _superDecoder(forKey key: CodingKey) throws -> Decoder {
        ...
        return _JSONMDecoder(referencing: value, at: self.decoder.codingPath, options: self.decoder.options)
    }
...
private struct _JSONUnkeyedDecodingContainer: UnkeyedDecodingContainer {
    ...
    // MARK: - Initialization
    /// Initializes `self` by referencing the given decoder and container.
    fileprivate init(referencing decoder: _JSONMDecoder, wrapping container: [JSON]) {
        self.decoder = decoder
        self.container = container
        self.codingPath = decoder.codingPath
        self.currentIndex = 0
...
extension _JSONMDecoder: SingleValueDecodingContainer {
