...
protocol SignInPresentationLogic: TablePresentationLogic, BioAuthPresentationLogic {
    func presentTable(_ response: SignIn.Table.Response)
}
...
class SignInPresenter: SignInPresentationLogic,
    ...
    private var screenType: SignIn.ScreenType?
    ...
    private func makeButton(for button: SignIn.Button) -> RoundCornersButton.Model {
    ...
    func presentTable(_ response: SignIn.Table.Response) {
        screenType = response.screen
        viewController?.displayTable(.init(sections: sections), withDiffer: true)
    }
}

extension CommonInputID {
    static let email = CommonInputID(rawValue: "Email")
    static let password = CommonInputID(rawValue: "Password")
    static let phone = CommonInputID(rawValue: "Phone")
}
